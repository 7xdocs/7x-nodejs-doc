...
for (const link of toc.match(/<a.*?>/g)) {
  ...
  // Split the doc.
  const match = /(<\/ul>\s*)?<\/\w+>\s*<\w+ role="main" id="apicontent">/.exec(
    data
  );
  ...
  apicontent +=
    ...
    data
      ...
      .replace(
        /<a class="mark" href="#([^"]+)" id="([^"]+)"/g,
        (match, anchor, id) => {
          if (anchor !== id)
            throw new Error(
              `Mark does not match: ${anchor} should match ${id}`
            );
...
all =
  all.slice(0, tocStart.index + tocStart[0].length) +
...
all =
  all
    .slice(0, apiStart.index + apiStart[0].length)
    .replace(
      '\n</head>',
      buildCSSForFlavoredJS(
        new Set(
          Array.from(
...
for (const match of hrefMatches) {
  if (!ids.has(match[1])) {
    ...
    //  throw new Error(`link not found: ${match[1]}`);
  }
}
